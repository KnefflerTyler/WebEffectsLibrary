...
// ── Layer state array ────────────────────────────────────────
const layers = [
  { el: document.getElementById('layer1'), preview: document.getElementById('preview1'), speed: 0.30, offset: 0, target: 0, blobUrl: null, defaultUrl: DEFAULT_IMG_1 },
  { el: document.getElementById('layer2'), preview: document.getElementById('preview2'), speed: 0.70, offset: 0, target: 0, blobUrl: null, defaultUrl: DEFAULT_IMG_2 },
];
...
// Layers are driven by window.scrollY so the effect only moves
// when the scrollbar is actually moving.
const LERP = 0.12; // smoothing factor (0 = frozen, 1 = instant)

// ── RAF render loop ──────────────────────────────────────────
function animate() {
  const scroll = window.scrollY;
  for (const layer of layers) {
    layer.target  = scroll * layer.speed;
    layer.offset += (layer.target - layer.offset) * LERP;
    layer.el.style.backgroundPositionY = `-${layer.offset.toFixed(2)}px`;
  }
...
function wireStaticLayer(index, speedId, speedValId, fileInputId, resetId) {
  ...
  document.getElementById(speedId).addEventListener('input', (e) => {
    ...
    document.getElementById(speedValId).textContent = layer.speed.toFixed(2) + '×';
  });
...
function createDynamicLayerCard(layer, num) {
  ...
  card.innerHTML = `
    ...
    <div class="speed-row">
      <span class="speed-label">Depth</span>
      <input type="range" id="speed_${uid}" min="0" max="2" step="0.05" value="0.5" aria-label="Layer ${num} depth">
      <span class="speed-value" id="speedVal_${uid}">0.50×</span>
    </div>
  ...
  card.querySelector(`#speed_${uid}`).addEventListener('input', (e) => {
    layer.speed = parseFloat(e.target.value);
    card.querySelector(`#speedVal_${uid}`).textContent = layer.speed.toFixed(2) + '×';
  });
...
addLayerBtn.addEventListener('click', () => {
  ...
  const layer = { el, preview: null, speed: 0.50, offset: 0, target: 0, blobUrl: null, defaultUrl: null };
  layers.push(layer);
