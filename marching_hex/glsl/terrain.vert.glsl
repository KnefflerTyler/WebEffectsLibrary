// ── Hex terrain vertex shader — GPU FBM displacement (GLSL ES 3.00 / WebGL 2) ─
// Three.js injects: modelMatrix, viewMatrix, projectionMatrix, normalMatrix,
// position (in vec3), normal (in vec3).
//
// Mesh geometry is a flat (y=0) hexagonal grid.  This shader evaluates seeded
// Perlin FBM noise to displace each vertex vertically and computes smooth
// normals via analytical central differences of the same noise function.
...
// ── 3-D Perlin noise with y fixed to 0 ───────────────────────────────────────
...
void main() {
    ...
    // ── Smooth normal via central differences ─────────────────────────────────
    float eps  = 1.0;
