// ── Hex Voxel terrain vertex shader — GPU FBM + quantization (GLSL ES 3.00 / WebGL 2)
// Mesh geometry is a flat (y=0) hexagonal XZ grid.  This shader evaluates
// seeded Perlin FBM noise, quantizes the result to uCellSize steps to produce
// a blocky stepped appearance over hex cells, then derives normals via central
// differences so the fragment shader can tell flat tops from step edges.
...
uniform float     uCellSize;    // vertical quantization step
...
void main() {
    ...
    // eps = uCellSize → flat tops give (0,1,0), step edges give steep tilt.
    float eps  = uCellSize;
